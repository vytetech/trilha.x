...
import { Repeat, Plus, Flame, Trash2, Zap } from "lucide-react";
...
export default function HabitsPage() {
  ...
  const createHabit = async () => {
    ...
    await supabase.from("habits").insert({ user_id: user.id, name, attribute, xp_reward: Number(xp) || 5 });
    setName(""); setAttribute("productivity"); setXp("5"); setDialogOpen(false);
    fetchData();
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="bg-card border-border">
            ...
            <div className="space-y-4">
              <div className="space-y-2"><Label>Nome</Label><Input value={name} onChange={(e) => setName(e.target.value)} className="bg-secondary border-border" placeholder="Ex: Treinar, Ler 20 páginas" /></div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2"><Label>XP por conclusão</Label><Input type="number" value={xp} onChange={(e) => setXp(e.target.value)} className="bg-secondary border-border" /></div>
              </div>
              <Button onClick={createHabit} className="w-full">Criar Hábito</Button>
            </div>
          </DialogContent>
        </Dialog>
      </motion.div>

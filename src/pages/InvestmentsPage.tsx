...
export default function InvestmentsPage() {
  ...
  const handleRefreshPrices = () => {
    fetchPrices(investments, fetchData);
  };

  // Quando abre o detalhe de um ativo, atualiza preço só daquele
  const handleRefreshSinglePrice = () => {
    if (!selectedInvestment) return;
    fetchPrices([selectedInvestment], fetchData);
  };
  ...
  // Usa preço ao vivo se disponível para os totais
  const getEffectivePrice = (inv: Investment) =>
    getLivePrice(inv.name)?.price ?? Number(inv.current_price);

  const totalInvested = investments.reduce(
    (a, i) => a + Number(i.quantity) * Number(i.average_price),
    0,
  );
  const totalCurrent = investments.reduce(
    (a, i) => a + Number(i.quantity) * getEffectivePrice(i),
    0,
  );
  const totalDividends = investments.reduce(
    (a, i) => a + Number(i.dividends_total),
    0,
  );
  ...
  const periodAportes = filteredTxs
    .filter((tx) => tx.type === "buy")
    .reduce((a, tx) => a + Number(tx.total), 0);
  const periodVendas = filteredTxs
    .filter((tx) => tx.type === "sell")
    .reduce((a, tx) => a + Number(tx.total), 0);
  const periodDividends = filteredTxs
  ...
  const createInvestment = async () => {
    ...
    if (!canCreate("investments", investments.length)) {
      toast({
        ...
        description: "Faça upgrade para adicionar mais investimentos.",
      });
      return;
    }
  ...
  const createInvestTx = async () => {
    if (!user || !txForm.investment_id || !txForm.quantity || !txForm.price)
      return;
    ...
    await supabase.from("investment_transactions").insert({
      user_id: user.id,
      investment_id: txForm.investment_id,
      type: txForm.type,
      quantity: qty,
      price,
      total,
    ...
    if (inv) {
      if (txForm.type === "buy") {
        ...
        const newAvg =
          (Number(inv.quantity) * Number(inv.average_price) + total) / newQty;
        await supabase
          .from("investments")
          .update({ quantity: newQty, average_price: newAvg })
          .eq("id", inv.id);
        ...
        await supabase
          .from("investments")
          .update({ dividends_total: Number(inv.dividends_total) + total })
          .eq("id", inv.id);
      }
    }
  ...
  const allocationData = Object.entries(
    investments.reduce(
      (acc, i) => {
        const t = typeLabels[i.asset_type];
        acc[t] = (acc[t] || 0) + Number(i.quantity) * getEffectivePrice(i);
        return acc;
      },
  ...
  const assetAllocation = investments
    .map((i) => ({
      name: i.name,
      value: Number(i.quantity) * getEffectivePrice(i),
      type: typeLabels[i.asset_type],
    }))
    .filter((a) => a.value > 0)
  ...
  const monthlyEvolution = useMemo(() => {
    const map: Record<
      string,
      { aportes: number; vendas: number; dividendos: number }
    > = {};
    filteredTxs.forEach((tx) => {
      const key = new Date(tx.transaction_date).toLocaleDateString("pt-BR", {
        month: "short",
        year: "2-digit",
      });
  ...
  const getInvestmentName = (id: string | null) =>
    investments.find((i) => i.id === id)?.name || "—";
  ...
  const topAssets = [...investments]
    .map((i) => {
      const lp = getLivePrice(i.name);
      const effPrice = lp?.price ?? Number(i.current_price);
      const inv = Number(i.quantity) * Number(i.average_price);
      return {
        name: i.name,
        value: Number(i.quantity) * effPrice,
        pl:
          inv > 0
            ? ((effPrice - Number(i.average_price)) / Number(i.average_price)) *
              100
            : 0,
        dailyChange: lp?.change ?? null,
      };
    })
    .sort((a, b) => b.value - a.value)
    .slice(0, 5);
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <PlanLimitBanner
        ...
        currentCount={investments.length}
        resourceLabel="investimentos"
      />
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <TrendingUp className="h-6 w-6 text-primary" /> Investimentos
          </h1>
          <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5">
            Acompanhe seu portfólio e evolução patrimonial
            {lastUpdated && (
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            title="Atualizar preços via brapi"
          >
            <RefreshCw
              className={`h-4 w-4 ${loadingPrices ? "animate-spin" : ""}`}
            />
          </Button>
            ...
            <DialogContent className="bg-card border-border">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-2">
                  <Button
                    variant={txForm.type === "buy" ? "default" : "outline"}
                  ...
                  <Button
                    variant={txForm.type === "sell" ? "default" : "outline"}
                  ...
                  <Button
                    variant={txForm.type === "dividend" ? "default" : "outline"}
                    onClick={() => setTxForm({ ...txForm, type: "dividend" })}
                    size="sm"
                  >
                    Dividendo
                  </Button>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setTxForm({ ...txForm, investment_id: v })
                    }
                  >
                  ...
                  <div className="p-3 rounded-lg bg-primary/10 border border-primary/20">
                    <p className="text-sm text-muted-foreground">
                      ...
                      <span className="font-bold text-foreground">
                        {fmt(Number(txForm.quantity) * Number(txForm.price))}
                      </span>
                    </p>
                  </div>
                )}
                <Button onClick={createInvestTx} className="w-full">
                  Registrar
                </Button>
              </div>
            </DialogContent>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="bg-secondary border-border"
                    placeholder="Ex: PETR4, KNRI11, BTC"
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Para cripto use o símbolo: BTC, ETH, SOL...
                  </p>
                </div>
                <div className="space-y-2">
                  <Label>Tipo</Label>
      ...
      <Tabs defaultValue="resumo">
        ...
        <TabsContent value="resumo" className="mt-4 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div
                  className={`rounded-lg p-1.5 ${totalPL >= 0 ? "bg-primary/10" : "bg-destructive/10"}`}
                >
                  {totalPL >= 0 ? (
              ...
              <p
                className={`text-xl font-bold font-mono ${totalPL >= 0 ? "text-primary" : "text-destructive"}`}
              >
                {fmt(totalPL)}
              </p>
              <p
                className={`text-[11px] font-mono mt-1 ${Number(rentPct) >= 0 ? "text-primary" : "text-destructive"}`}
              >
                {Number(rentPct) >= 0 ? "+" : ""}
                {rentPct}%
              </p>
            ...
            {monthlyEvolution.length > 0 && (
              <div className="rounded-xl border border-border bg-card p-5">
                <h3 className="text-sm font-semibold text-foreground mb-4">
                  Aportes vs Vendas vs Dividendos
                </h3>
          ...
          {/* Top assets com variação diária */}
          {topAssets.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-5">
              <h3 className="text-sm font-semibold text-foreground mb-4">
                Maiores Posições
                ...
                {topAssets.map((a, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {/* P&L total */}
                      <span
                        className={`text-xs font-mono ${a.pl >= 0 ? "text-primary" : "text-destructive"}`}
                      >
                        {a.pl >= 0 ? "+" : ""}
                        {a.pl.toFixed(1)}%
                      </span>
                      {/* Variação do dia (só se tiver preço ao vivo) */}
                      {a.dailyChange !== null && (
                        <span
                          className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${a.dailyChange >= 0 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
                        >
                          {a.dailyChange >= 0 ? "▲" : "▼"}{" "}
                          {Math.abs(a.dailyChange).toFixed(2)}% hoje
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <TabsContent value="ativos" className="mt-4">
          <div className="space-y-2">
            {investments.map((inv) => {
              const lp = getLivePrice(inv.name);
              const effPrice = lp?.price ?? Number(inv.current_price);
              const inv_invested =
                Number(inv.quantity) * Number(inv.average_price);
              const current = Number(inv.quantity) * effPrice;
              const pl = current - inv_invested;
              const pct =
                inv_invested > 0 ? ((pl / inv_invested) * 100).toFixed(2) : "0";
              return (
                <div
                  key={inv.id}
                  onClick={() => {
                ...
                >
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-0.5 flex-wrap">
                        <Badge variant="outline" className="text-xs">
                          {typeLabels[inv.asset_type]}
                        </Badge>
                        <span className="text-xs text-muted-foreground">
                          {Number(inv.quantity)} un
                        </span>
                        <span className="text-xs text-muted-foreground">
                          PM: {fmt(Number(inv.average_price))}
                        </span>
                        <span className="text-xs text-muted-foreground">
                          Atual: {fmt(effPrice)}
                        </span>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <p
                        className={`text-xs font-mono ${Number(pct) >= 0 ? "text-primary" : "text-destructive"}`}
                      >
                        {Number(pct) >= 0 ? "+" : ""}
                        {pct}%
                      </p>
                    </div>
                    {/* Badge variação do dia */}
                    {lp && (
                      <div
                        className={`hidden sm:flex items-center gap-1 text-[11px] font-mono px-2 py-1 rounded-md ${lp.change >= 0 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
                      >
                        {lp.change >= 0 ? (
        ...
        <TabsContent value="transacoes" className="mt-4 space-y-2">
          {investTxs.map((tx) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {getInvestmentName(tx.investment_id)}
                  </p>
                  <div className="flex gap-2 mt-0.5 flex-wrap">
                    <Badge variant="outline" className="text-xs">
                      {tx.type === "buy"
                    ...
                    <span className="text-xs text-muted-foreground">
                      {Number(tx.quantity)} un × {fmt(Number(tx.price))}
                    </span>
                    <span className="text-xs text-muted-foreground">
              ...
              <span
                className={`font-bold font-mono ${tx.type === "sell" ? "text-primary" : tx.type === "dividend" ? "text-accent" : "text-foreground"}`}
              >
                {fmt(Number(tx.total))}
              </span>
            </div>
        ...
        <TabsContent value="dividendos" className="mt-4 space-y-6">
              ...
              <p className="text-2xl font-bold text-foreground font-mono">
                {fmt(
                  dividendEvolution.length > 0
                    ? totalDividends / dividendEvolution.length
                    : 0,
                )}
              </p>
            </div>
          ...
          {dividendEvolution.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-5">
              <h3 className="text-sm font-semibold text-foreground mb-4">
                Evolução dos Dividendos (Acumulado)
              </h3>
                  ...
                  <AreaChart data={dividendEvolution}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v: number, name: string) => [
                        fmt(v),
                        name === "cumulative" ? "Acumulado" : "Mensal",
                      ]}
                    />
                    <Area
                      type="monotone"
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dividendEvolution}>
                    ...
                    <Tooltip
                      contentStyle={tooltipStyle}
                      cursor={customCursor}
                      formatter={(v: number) => [fmt(v), "Mensal"]}
                    />
                    <Bar
                      dataKey="monthly"
                      fill="hsl(153 80% 40%)"
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-2">
                {dividendByAsset.map((a, i) => {
                  const pctOfTotal =
                    totalDividends > 0 ? (a.value / totalDividends) * 100 : 0;
                  return (
        ...
        <TabsContent value="alocacao" className="mt-4 space-y-6">
          {allocationData.length > 0 ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>Adicione ativos para ver a alocação.</p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="simulador" className="mt-4">
          <div className="rounded-xl border border-border bg-card p-6 max-w-lg">
            <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <Calculator className="h-5 w-5 text-primary" /> Simulador de
              Aportes
            </h3>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={simAporte}
                  onChange={(e) => setSimAporte(e.target.value)}
                  className="bg-secondary border-border"
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  value={simAnos}
                  onChange={(e) => setSimAnos(e.target.value)}
                  className="bg-secondary border-border"
                />
              </div>
              <div className="mt-4 p-4 rounded-lg bg-primary/10 border border-primary/30">
                <p className="text-sm text-muted-foreground">
                  Resultado estimado em {simAnos} anos:
                </p>
                <p className="text-3xl font-bold neon-text font-mono mt-1">
                  {fmt(simResult)}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Total aportado:{" "}
                  {fmt(Number(simAporte) * Number(simAnos) * 12)}
                </p>
              </div>
            </div>
          </div>
      ...
      <InvestmentDetailSheet
        ...
        livePrice={
          selectedInvestment ? getLivePrice(selectedInvestment.name) : null
        }
        onRefreshPrice={handleRefreshSinglePrice}

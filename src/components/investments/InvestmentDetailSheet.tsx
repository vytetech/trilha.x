...
import { type LivePrice } from "@/hooks/useInvestmentPrices";
...
export default function InvestmentDetailSheet({
  ...
  const displayPrice = livePrice?.price ?? Number(investment.current_price);
  const invested =
    Number(investment.quantity) * Number(investment.average_price);
  const current = Number(investment.quantity) * displayPrice;
      ...
      <SheetContent className="bg-card border-border w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader className="pb-4">
          <SheetTitle className="flex items-center gap-2 text-foreground">
            ...
            <Badge variant="outline" className="ml-1">
              {typeLabels[investment.asset_type]}
            </Badge>
            {onRefreshPrice && (
              <Button
                variant="ghost"
              ...
              >
                <RefreshCw
                  className={`h-3.5 w-3.5 ${loadingPrice ? "animate-spin" : ""}`}
                />
              </Button>
            )}
          </SheetTitle>
        ...
        {livePrice && (
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 mb-4 flex items-center justify-between">
            <div>
              ...
              {livePrice.updatedAt && (
                <p className="text-[10px] text-muted-foreground mt-1">
                  Atualizado:{" "}
                  {new Date(livePrice.updatedAt).toLocaleTimeString("pt-BR", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </p>
            ...
            <div
              className={`flex items-center gap-1.5 rounded-lg px-3 py-2 ${dailyPositive ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
            >
              {dailyPositive ? (
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="rounded-lg border border-border bg-secondary/50 p-3">
            ...
            <p className="text-lg font-bold font-mono text-foreground">
              {fmt(Number(investment.average_price))}
            </p>
          </div>
          <div className="rounded-lg border border-border bg-secondary/50 p-3">
          ...
          <div className="rounded-lg border border-border bg-secondary/50 p-3">
            ...
            <p
              className={`text-lg font-bold font-mono ${isPositive ? "text-primary" : "text-destructive"}`}
            >
              {isPositive ? "+" : ""}
              {pct}%
            </p>
          ...
          <div className="rounded-lg border border-border bg-secondary/50 p-3">
            ...
            <p
              className={`text-base font-bold font-mono ${isPositive ? "text-primary" : "text-destructive"}`}
            >
              {fmt(current)}
            </p>
          </div>
        ...
        {/* P&L em reais */}
        <div
          className={`rounded-lg border p-3 mb-6 flex items-center justify-between ${isPositive ? "border-primary/20 bg-primary/5" : "border-destructive/20 bg-destructive/5"}`}
        >
          <p className="text-xs text-muted-foreground">Lucro / Prejuízo (R$)</p>
          <p
            className={`text-xl font-bold font-mono ${isPositive ? "text-primary" : "text-destructive"}`}
          >
            {isPositive ? "+" : ""}
            {fmt(pl)}
          </p>
        ...
        {Number(investment.dividends_total) > 0 && (
          <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 mb-6">
            ...
            <p className="text-lg font-bold font-mono text-primary">
              {fmt(Number(investment.dividends_total))}
            </p>
          </div>
        )}
                ...
                <AreaChart data={positionData}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number, name: string) => [
                      name === "invested" ? fmt(v) : v,
                      name === "invested" ? "Investido" : "Quantidade",
                    ]}
                  />
                  <Area
                    type="monotone"
